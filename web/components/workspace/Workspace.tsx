...
export function Workspace() {
  ...
  // Lifted out of `PolicyProfile` so the suggested questions can be built from
  // it. Cleared when the document changes: suggestions from the previous
  // policy are questions this one may not answer.
  const [profile, setProfile] = useState<PolicyProfileData | null>(null);
  ...
  const selectDocument = useCallback((document: DocumentSummary) => {
    ...
    // Suggestions are claims about *this* document. Carrying the previous
    // one's over would offer questions the new policy may not answer.
    setProfile(null);
  }, []);
  ...
  // Written by hand for the three bundled samples, derived from the extracted
  // profile for everything else.
  //
  // Not a fallback in the apologetic sense: the derived ones are the general
  // case and the hand-written ones are a curated exception for the documents
  // every visitor sees first. Where both exist the hand-written ones win —
  // they read better, and a demo's first screen is worth the words.
  const derived = useMemo(
    () => suggestQuestions(profile, locale).map((s) => s.question),
    [profile, locale],
  );
              ...
              <div className="px-1 pt-6">
                ...
                {suggestions.length > 0 && (
                  <div className="mt-5">
                    ...
                    {/* Said out loud when the list was built from the document
                        rather than written by us. Each of these questions
                        corresponds to a slot typed extraction actually filled,
                        so every one of them is answerable — which is the whole
                        reason they are not generated by a model. */}
                    {suggestions === derived && (
                      <p className="mt-1 text-[11px] leading-4 text-ink-faint">
                        {t.workspace.suggestionsDerived}
                      </p>
                    )}
